...
<head>
  ...
  <title>Mascotas | Pawsitive</title>
...
<body>
  ...
  <span class="header mt-5 text-light fw-bold">Mascotas
    <a href="../forms/addMascota.php"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-plus-square-fill icon add" viewBox="0 0 16 16">
    ...
    <tbody>
      <?php foreach ($mascotas as $masc) {
        ...
        foreach ($allClients as $c) {
          if ($c['id'] == $masc['cliente_id']) {
            $cliente = $c['nombre_apellido'];
            $clienteID = $c['id'];
          }
        }
      ?>
        <tr>
          ...
          <td>
            <!-- EDITAR MASCOTA -->
            <a href=<?php echo str_replace(' ', '%20', '../forms/editMascotya.php?nombre=' . $masc['nombre'] . '&tipo=' . $masc['tipo'] . '&color=' . $masc['color'] . '&edad=' . $masc['edad'] . '&sexo=' . $masc['sexo'] . '&mascota_id=' . $masc['id'] . '&cliente_id=' . $clienteID) ?>>
